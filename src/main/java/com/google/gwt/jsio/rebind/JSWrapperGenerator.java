...
public class JSWrapperGenerator extends Generator {
	...
	/**
	 * The name of the static field that contains the class's Extractor
	 * instance.
	 */
	 ...
	 * target node, the target's metadata will be examined for a tag based on
	 * the requested Annotation's {@link MetaDataName} meta-annotation. If the
	 * target has metadata which can be interpreted as the return type of the
	 * requested Annotation's value method, a {@link Proxy} will be synthesized.
	 * The proxy mode is only to support existing functionality, all new
	 * features should be added via new annotations.
	 * 
	 * @param <A>
	 ...
	 *             if metadata with the correct tag exists but cannot be
	 *             interpreted as the return type of the annotation's value
	 *             method
	 */
	...
	static <A extends Annotation, M extends HasAnnotations & HasMetaData> A hasTag(TreeLogger logger, M target, final Class<A> annotation) throws UnableToCompleteException {
		...
		if (metaDataName == null) {
			// Indicates that the requested annotation doesn't have legacy
			// support
	...
	public final String generate(TreeLogger logger, GeneratorContext context, java.lang.String typeName) throws UnableToCompleteException {
		...
		if (out != null) {
			...
			fragmentContext.initProperties = !(hasTag(logger, sourceType, NoInitProperties.class) != null);
			...
			if (!fragmentContext.readOnly) {
				// TODO verificar as implicações dessa altweração.
	...
	protected void writeBoilerplate(TreeLogger logger, FragmentGeneratorContext context) throws UnableToCompleteException {
		...
		// Method declaration
		sw.println("public native " + context.simpleTypeName + " setJavaScriptObjectNative(JavaScriptObject obj) /*-{");
		sw.indent();
		...
		// If the generated class will be used with a JSList, we need an
		// Extractor
	 ...
	 * return the field value, without having to check it for an undefined
	 * value.
	 */
	protected void writeEmptyFieldInitializers(FragmentGeneratorContext context) throws UnableToCompleteException {
		...
		for (Task task : context.tasks) {
			...
			stringBuffer.append(context.parameterName);
			sw.print(".hasOwnProperty('");
			stringBuffer.append(".hasOwnProperty('");
			sw.print(fieldName);
			...
			stringBuffer.append(fragmentGenerator.defaultValue(context.typeOracle, returnType));
			sw.println(";");
			stringBuffer.append(";");
		...
		System.out.println("JSWrapperGenerator.writeEmptyFieldInitializers()" + stringBuffer.toString());
	}
	...
	protected void writeFixups(TreeLogger logger, TypeOracle typeOracle, SourceWriter sw, Set<JClassType> creatorFixups) throws UnableToCompleteException {
		for (JClassType asClass : creatorFixups) {
			// If the type is parameterized, we want to replace it with the raw
			// type
	...
	protected void writeImported(FragmentGeneratorContext context, JMethod imported) throws UnableToCompleteException {
		...
		// Wrap the return type in the correct Java type. Void returns are
		// ignored
		if (!JPrimitiveType.VOID.equals(returnType.isPrimitive())) {
			FragmentGeneratorContext subContext = new FragmentGeneratorContext(context);
			subContext.returnType = returnType;
	 ...
	 * Also write BusObjectImpl methods for Map-style access.
	 */
