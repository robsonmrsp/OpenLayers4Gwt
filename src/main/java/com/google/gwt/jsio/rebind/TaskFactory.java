...
public class TaskFactory {
	...
	public static interface Policy {
		 ...
		 */
		boolean shouldBind(TreeLogger logger, TypeOracle oracle, JMethod m) throws UnableToCompleteException;

		/**
		 * Determines if a method should be treated as an invocation of an
		 * underlying JavaScript constructor function.
		 */
		boolean shouldConstruct(TreeLogger logger, TypeOracle oracle, JMethod m) throws UnableToCompleteException;

		/**
		 * Determines if the generator should generate an export binding for the
		 * method.
		 */
		boolean shouldExport(TreeLogger logger, TypeOracle oracle, JMethod m) throws UnableToCompleteException;

		/**
		 * Determines if the generator should implement a particular method. A
		 * method will be implemented only if it is abstract and defined in a
		 * class derived from JSWrapper
		 */
		boolean shouldImplement(TreeLogger logger, TypeOracle oracle, JMethod m) throws UnableToCompleteException;

		/**
		 * Determines if the generator should generate an import binding for the
		 * method.
		 */
		boolean shouldImport(TreeLogger logger, TypeOracle oracle, JMethod m) throws UnableToCompleteException;
	}
	...
	private static class ExporterPolicy implements Policy {
		public Collection<JMethod> getOperableMethods(TypeOracle oracle, JClassType clazz) {
			...
			for (JClassType searchIn : stack) {
				for (JMethod m : searchIn.getMethods()) {
					// We add a stripped declaration so that changes which don't
					// affect
					// the overall signature will be overwritten by the methods
					// in the
					// leaf type.
					toReturn.put(m.getReadableDeclaration(true, true, true, true, true), m);
				}
			}
		...
		public boolean shouldConstruct(TreeLogger logger, TypeOracle oracle, JMethod m) {
			return false;
		}

		public boolean shouldExport(TreeLogger logger, TypeOracle typeOracle, JMethod method) throws UnableToCompleteException {
			return JSWrapperGenerator.hasTag(logger, method, Exported.class) != null;
		}

		public boolean shouldImplement(TreeLogger logger, TypeOracle oracle, JMethod m) {
			return false;
		}
	...
	private static class FlyweightPolicy extends WrapperPolicy {
		@Override
		public boolean shouldBind(TreeLogger logger, TypeOracle typeOracle, JMethod method) throws UnableToCompleteException {

			boolean hasBindingTag = JSWrapperGenerator.hasTag(logger, method, Binding.class) != null;
			JParameter[] params = method.getParameters();

			return method.isAbstract() && hasBindingTag && ((params.length == 1) || (params.length == 2)) && isJsoOrPeer(typeOracle, params[0].getType())
					&& ((params.length == 1) || (params[1].getType().isClassOrInterface() != null));
		}

		@Override
		public boolean shouldImport(TreeLogger logger, TypeOracle typeOracle, JMethod method) throws UnableToCompleteException {
			JClassType enclosing = method.getEnclosingType();
			String methodName = method.getName();
			int arguments = method.getParameters().length;

			boolean hasBindingTag = JSWrapperGenerator.hasTag(logger, method, Binding.class) != null;
			boolean hasImportTag = JSWrapperGenerator.hasTag(logger, method, Imported.class) != null;
			boolean methodHasBeanTag = JSWrapperGenerator.hasTag(logger, method, BeanProperties.class) != null;
			boolean classHasBeanTag = JSWrapperGenerator.hasTag(logger, enclosing, BeanProperties.class) != null;

			boolean isIs = (arguments == 1) && (methodName.startsWith("is")) && (JPrimitiveType.BOOLEAN.equals(method.getReturnType().isPrimitive()))
					&& isJsoOrPeer(typeOracle, method.getParameters()[0].getType());
			boolean isGetter = (arguments == 1) && (methodName.startsWith("get") && isJsoOrPeer(typeOracle, method.getParameters()[0].getType()));
			boolean isSetter = (arguments == 2) && (methodName.startsWith("set") && isJsoOrPeer(typeOracle, method.getParameters()[0].getType()));
			boolean propertyAccessor = isIs || isGetter || isSetter;

			return !(hasBindingTag || methodHasBeanTag || (propertyAccessor && !hasImportTag && classHasBeanTag));
		}
		...
		protected boolean isJsoOrPeer(TypeOracle oracle, JType type) {
			...
			return jsoType.isAssignableFrom(type.isClass()) || (PeeringFragmentGenerator.findPeer(oracle, type) != null);
		}
	}
	...
	private static class WrapperPolicy implements Policy {
		public Collection<JMethod> getOperableMethods(TypeOracle typeOracle, JClassType clazz) {
		...
		public boolean shouldConstruct(TreeLogger logger, TypeOracle typeOracle, JMethod method) throws UnableToCompleteException {
			boolean methodConstructorTag = JSWrapperGenerator.hasTag(logger, method, Constructor.class) != null;
			boolean methodGlobalTag = JSWrapperGenerator.hasTag(logger, method, Global.class) != null;
		...
		public boolean shouldImplement(TreeLogger logger, TypeOracle typeOracle, JMethod method) throws UnableToCompleteException {
		...
		public boolean shouldImport(TreeLogger logger, TypeOracle typeOracle, JMethod method) throws UnableToCompleteException {
			...
			boolean methodHasBeanTag = JSWrapperGenerator.hasTag(logger, method, BeanProperties.class) != null;
			boolean classHasBeanTag = JSWrapperGenerator.hasTag(logger, enclosing, BeanProperties.class) != null;
			boolean isIs = (arguments == 0) && (methodName.startsWith("is")) && (JPrimitiveType.BOOLEAN.equals(method.getReturnType().isPrimitive()));
	 ...
	 */
	public static Map<String, Task> extractMethods(TreeLogger logger, TypeOracle typeOracle, JClassType clazz, Policy policy) throws UnableToCompleteException {
		TreeLogger parentLogger = logger.branch(TreeLogger.DEBUG, "Extracting methods from " + clazz.getName(), null);
		...
		for (JMethod m : policy.getOperableMethods(typeOracle, clazz)) {
			...
			logger = parentLogger.branch(TreeLogger.DEBUG, "Examining " + m.toString(), null);
			...
			if (policy.shouldExport(logger, typeOracle, m)) {
				Task task = getPropertyPair(propertyAccessors, m.getReadableDeclaration());
				task.exported = m;
			...
			// Ignore concrete methods and those methods that are not declared
			// in
			...
			if (policy.shouldConstruct(logger, typeOracle, m)) {
				// getReadableDeclaration is used so that overloaded methods
				// will
				...
				// Enable bypassing of name-determination logic with the
				// presence of the
				...
				// We could not make a decision on what should be done with the
				// method.
			} else {
				logger.log(TreeLogger.ERROR, "Could not decide on implementation of " + m.getName(), null);
	 ...
	 * 
	 * @return The property name if the method's name looks like a bean
	 *         property, otherwise the method's name.
	 */
	 ...
	 */
	protected static Task getPropertyPair(Map<String, Task> propertyAccessors, String property) {
