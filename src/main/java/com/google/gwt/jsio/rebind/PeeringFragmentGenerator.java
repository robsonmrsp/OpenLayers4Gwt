...
class PeeringFragmentGenerator extends JSWrapperFragmentGenerator {
	...
	static JField findPeer(TypeOracle oracle, JType type) {
		...
		while (asClass != null) {
			...
			if (f != null && isAssignable(oracle, f.getType().isClassOrInterface(), JavaScriptObject.class)) {
				return f;
			}
	...
	static JMethod findConstructor(TypeOracle oracle, JType type) {
		...
		JMethod m = asClass.findMethod(JSFlyweightWrapperGenerator.CREATE_PEER, new JType[] { oracle.findType(JavaScriptObject.class.getName()) });
	...
	static JField findExtractor(TypeOracle oracle, JType type) {
		...
		while (asClass != null) {
			...
			if (f != null && isAssignable(oracle, f.getType().isClassOrInterface(), Extractor.class)) {
				return f;
			}
			asClass = asClass.getSuperclass();
		}
	...
	@Override
	protected void writeJSNIObjectCreator(FragmentGeneratorContext context) throws UnableToCompleteException {
		if (findConstructor(context.typeOracle, context.returnType) == null) {
			context.parentLogger.log(TreeLogger.ERROR, "The type " + context.returnType.getQualifiedSourceName() + " must possess a " + JSFlyweightWrapperGenerator.CREATE_PEER
	...
	@Override
	protected void writeJSNIValue(FragmentGeneratorContext context) throws UnableToCompleteException {
		JField f = findPeer(context.typeOracle, context.returnType);
		if (f == null) {
			context.parentLogger.log(TreeLogger.ERROR, "The type or a supertype of " + context.returnType.getQualifiedSourceName() + " must possess a " + JSWrapperGenerator.OBJ
	...
	boolean accepts(TypeOracle oracle, JType type) {
		return (findConstructor(oracle, type) != null) || (findExtractor(oracle, type) != null) || (findPeer(oracle, type) != null);
	}
	...
	void writeExtractorJSNIReference(FragmentGeneratorContext context) throws UnableToCompleteException {
		...
		if (f == null) {
			context.parentLogger.log(TreeLogger.ERROR, "The type or a supertype of " + context.returnType.getQualifiedSourceName() + " must possess a "
					+ JSWrapperGenerator.EXTRACTOR + " field to be used with a JSList", null);
			throw new UnableToCompleteException();
		}
